...
__declspec(dllexport) void pairwise_gpu(Eigen::ArrayXf *force_x, Eigen::ArrayXf *force_y, Eigen::ArrayXf atoms_x, Eigen::ArrayXf atoms_y, float SD_factor, int threads_per_block)
{
	...
	float alpha = 1.f;
	float beta = 0.f;
	cublasSgemv(handle, CUBLAS_OP_T, N, N, &alpha, thrust::raw_pointer_cast(diffs_x_d), N,
		thrust::raw_pointer_cast(d_ones.data()), 1, &beta, thrust::raw_pointer_cast(force_x_d), 1);
	cublasSgemv(handle, CUBLAS_OP_T, N, N, &alpha, thrust::raw_pointer_cast(diffs_y_d), N,
		thrust::raw_pointer_cast(d_ones.data()), 1, &beta, thrust::raw_pointer_cast(force_y_d), 1);
	...
	check(cudaMemcpy(force_x_h, force_x_d, bytes, cudaMemcpyDeviceToHost));
	check(cudaMemcpy(force_y_h, force_y_d, bytes, cudaMemcpyDeviceToHost));

	*force_x = Eigen::Map<Eigen::ArrayXf>(force_x_h, N);
	*force_y = Eigen::Map<Eigen::ArrayXf>(force_y_h, N);
