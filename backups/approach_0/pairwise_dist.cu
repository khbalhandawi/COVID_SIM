...
__declspec(dllexport) void pairwise_gpu(Eigen::ArrayXf *force_x, Eigen::ArrayXf *force_y, Eigen::ArrayXf atoms_x, Eigen::ArrayXf atoms_y, float SD_factor, int threads_per_block)
{
	...
	calc_forces << <gridSize, blockSize >> > (force_x_d, diffs_x_d, N);
	calc_forces << <gridSize, blockSize >> > (force_y_d, diffs_y_d, N);
	
	//// --- Matrix allocation and initialization
	//thrust::device_vector<float> d_matrix(*diffs_x_d);

	//// --- Allocate space for row sums and indices
	//thrust::device_vector<float> d_row_sums(N);
	//thrust::device_vector<int> d_row_indices(N);
