...
def draw_time_series(time, data, label, fig, ax1, 
    line_label=None, threshold=None, threshold_label=None,
    palette = ['#1C758A', '#CF5044', '#BBBBBB', '#444444'],
    plot_path = '/render'):
    ...
    check_folder(plot_path)
    fig.savefig('%s/%s_plot.pdf' %(plot_path, label), dpi=100, facecolor=bg_color, bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    points = np.array( [[30.0, 0.075, 10],
                        [60.0, 0.075, 10],
                        [30.0, 0.150, 10],
                        [30.0, 0.075, 20]])

    run = 0; data_I = []; data_F = []; data_R = []; data_M = []; labels = []
    for point in points:
        ...
        data_I += [I]; data_F += [F]; data_R += [R]; data_M += [M]
    ...
    draw_SIR_compare(data_I, fig_1, ax1_1, labels=labels, palette = palette, 
        pop_size = pop_size, save_name = 'I_compare', xlim = 350, ylim = 450, 
    ...
    draw_SIR_compare(data_F, fig_2, ax1_2, labels=labels, palette = palette, 
        pop_size = pop_size, save_name = 'F_compare', xlim = 350, ylim = 75, 
    ...
    draw_SIR_compare(data_R, fig_3, ax1_3, labels=labels, palette = palette, 
        pop_size = pop_size, save_name = 'R_compare', xlim = 350, ylim = 900, 
