...
for i = 1:size(z,1)
    ...
    format = [repmat('%12.20f,',1, length(x) + ceal{4} + 4) '\n']; format(end-2) = ''; % remove last delimliter
    fprintf(hist_file, format, hist_avg(end,:)');
    fclose(hist_file); 

    % store feasible improvement history
    if isempty(x_feas) == 0
        [f_feas,c_feas,p_feas,h_feas,u_feas] = get_updated_estimates(hist(:,2:end),x_feas,nprob);
        f_hist = [f_hist; [bbe, x_feas', f_feas, c_feas, p_feas, h_feas] ];
    else
        [f_feas,c_feas,p_feas,h_feas,u_feas] = get_updated_estimates(hist(:,2:end),x_infeas,nprob);
    ...
    [f_infeas,c_infeas,p_infeas,h_infeas,u_infeas] = get_updated_estimates(hist(:,2:end),x_infeas,nprob);
    i_hist = [i_hist; [bbe, x_infeas', f_infeas, c_infeas, p_infeas, h_infeas, u_infeas] ];
